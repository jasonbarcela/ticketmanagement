...
async function findStatusById(ticketId) {
  const [rows] = await pool.execute(
    'SELECT ticket_id, status FROM repair_tickets WHERE ticket_id = ?',
    [ticketId]
  );
  return rows[0] || null;
}
...
async function create(fields) {
  ...
  try {
    ...
    const {
      ...
      preferred_schedule, assigned_tech, estimated_cost, received_date
    } = fields;
    ...
    const [ticketResult] = await conn.execute(
      `INSERT INTO repair_tickets
         (ticket_number, customer_id, device_id, booking_id, problem_desc, 
          service_type, address, preferred_schedule, assigned_tech, 
...
async function update(ticketId, fields) {
  ...
  try {
    ...
    const {
      ...
      assigned_tech, diagnostic_notes, repair_notes, additional_findings,
      customer_approval, estimated_cost, status, completed_date
    } = fields;
    ...
    await conn.execute(
      `UPDATE repair_tickets SET
         problem_desc = ?, service_type = ?, address = ?, preferred_schedule = ?,
         assigned_tech = ?, diagnostic_notes = ?, repair_notes = ?, 
         additional_findings = ?, customer_approval = ?, estimated_cost = ?, 
         status = ?, completed_date = ?
       WHERE ticket_id = ?`,
      [
        problem_desc, service_type, address, preferred_schedule,
        assigned_tech, diagnostic_notes, repair_notes,
        additional_findings, customer_approval || null, parseFloat(estimated_cost) || 0.00,
        status, completed_date || null,
        ticketId
      ]
    );
